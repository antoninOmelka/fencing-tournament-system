...
export function useParticipants() {
    ...
    const editParticipant = useCallback(async (id: number, data: ParticipantInputs) => {
        try {
            const updatedParticipant = { ...data, id };
            await updateParticipant(updatedParticipant);
            setParticipants((prev) =>
                prev.map((participant) => (participant.id === id ? updatedParticipant : participant))
            );
        } catch (error) {
    ...
    return { participants, isLoading, addParticipant, editParticipant, removeParticipant };
}
